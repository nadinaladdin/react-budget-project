...
const MonthDay = ({children, clicked, isNotVisibleMonth = false, isSelected = false}) => {
    return (
...
export default class DatePicker extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            visibleDate: this.props.selectedDate ? this.props.selectedDate : new Date(new Date().setHours(0,0,0,0))
        };
    }
    ...
    render() {
        const {defaultValue, selectedDate} = this.props;
        const {currentDate, visibleDate} = this.state;
        const parsedDate = this.getParsedDate(visibleDate ? visibleDate : currentDate);
        ...
        for (let i = 0; i < VISIBLE_AMOUNT_OF_DAYS; i++) {
            const date = new Date(visibleDate ? visibleDate : currentDate);
            if (i < dayOffset) {
                monthDays.push({
            ...
            <div className="datepicker__wrapper">
                ...
                <div className={`datepicker__container ${this.state.isOpen ? 'datepicker__container_open' : ''}`}>
                    <div className="datepicker__calendar-menu">
                        <div className="datepicker__calendar-label">{MONTH_NAME[`${parsedDate.month}`]} {parsedDate.year}</div>
